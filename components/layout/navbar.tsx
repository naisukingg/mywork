    ...
    <nav className={`fixed inset-x-0 top-0 z-[1000] flex items-center bg-transparent px-6 py-6 sm:px-10 lg:px-20 [font-family:'Space_Grotesk',sans-serif] ${isAuthPage ? 'justify-start' : 'justify-between'}`}>
      ...
      {!isAuthPage && user && !loading && (
        <button
          ...
          className="ml-3 rounded-[14px] bg-transparent px-5 py-3 text-sm font-bold text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.25)] transition duration-200 hover:bg-white/10"
        >
          Log out
        </button>
      )}
